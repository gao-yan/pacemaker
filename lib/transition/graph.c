...
action_overrun(crm_graph_t * graph, synapse_t * synapse)
{
    ...
    for (lpc = synapse->actions; lpc != NULL; lpc = lpc->next) {
        ...
        /* Defaults to 0, the limit will be twice the number of CPU cores on DC, or 4 */
        if (value == NULL || crm_int_helper(value, NULL) == 0) {
            actions_limit = num_cpus > 0 ? num_cpus * 2 : 4;
        ...
        /* Defaults to 0, the limit will be half the number of CPU cores on DC, or 1 */
        if (value == NULL || crm_int_helper(value, NULL) == 0) {
