...
/* The ticket configuration section:
 * "/cib/configuration/tickets/ticket" */
gboolean
unpack_tickets(xmlNode * xml_tickets, pe_working_set_t * data_set)
{
    ...
    for (xml_obj = __xml_first_child(xml_tickets); xml_obj != NULL; xml_obj = __xml_next(xml_obj)) {
        ...
        if (crm_str_eq((const char *)xml_obj->name, XML_CIB_TAG_TICKET, TRUE) == FALSE) {
            continue;
        }

        ticket_id = ID(xml_obj);

        if (ticket_id == NULL || strlen(ticket_id) == 0) {
            crm_config_err("Must specify id tag in <ticket>");
            continue;
        }

        crm_trace("Processing ticket %s", ticket_id);
        ...
        /*unpack_instance_attributes(data_set->input, xml_obj, XML_TAG_ATTR_SETS, NULL,
                                   ticket->config, NULL, FALSE, data_set->now);*/
        /*Choice 2*/
        for (xIter = xml_obj->properties; xIter; xIter = xIter->next) {
            ...
            g_hash_table_replace(ticket->config, crm_strdup(prop_name), crm_strdup(prop_value));
        }

        standby = g_hash_table_lookup(ticket->config, "standby");
        if (standby && crm_is_true(standby)) {
            crm_info("Ticket %s is in standby-mode", ticket->id);
            ticket->standby = TRUE;
        } else {
            ticket->standby = FALSE;
        }

    crm_trace("Done with ticket %s", ticket_id);
    }
...
unpack_status(xmlNode * status, pe_working_set_t * data_set)
{
    ...
    for (state = __xml_first_child(status); state != NULL; state = __xml_next(state)) {
        if (crm_str_eq((const char *)state->name, XML_CIB_TAG_TICKETS, TRUE)) {
            ...
            /* Compatibility with the deprecated ticket state section:
             * Unpack the arrtibutes in the deprecated "/cib/status/tickets/instance_attributes" if it exists. */
