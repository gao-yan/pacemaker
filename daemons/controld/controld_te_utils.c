...
static void
init_node_pending_timer(const char *node_uuid, guint timeout)
{
    struct abort_timer_s *node_pending_timer = NULL;
    char *key = NULL;
    ...
    node_pending_timer = g_hash_table_lookup(node_pending_timers, node_uuid);
    if (node_pending_timer != NULL) {
        return;
    }
...
controld_node_pending_timer(crm_node_t *node)
{
    ...
    init_node_pending_timer(node->uuid, remaining_timeout);
}
