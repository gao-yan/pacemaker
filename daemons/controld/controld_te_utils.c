...
node_pending_timer_popped(gpointer key)
{
    ...
    crm_warn("Node with id=%u pending timed out on joining the process group "
             "(%us)",
             GPOINTER_TO_UINT(key), controld_globals.node_pending_timeout);
...
static void
init_node_pending_timer(uint32_t node_id, guint timeout)
{
    struct abort_timer_s *node_pending_timer = NULL;
    gpointer key = NULL;

    if (node_pending_timers == NULL) {
        node_pending_timers = pcmk__intkey_table(free_node_pending_timer);
    }

    key = GUINT_TO_POINTER(node_id);
    node_pending_timer = g_hash_table_lookup(node_pending_timers, key);
    if (node_pending_timer != NULL) {
        return;
    }
    ...
    g_hash_table_insert(node_pending_timers, key, node_pending_timer);
}

static void
remove_node_pending_timer(uint32_t node_id)
{
    if (node_pending_timers == NULL) {
        return;
    }

    g_hash_table_remove(node_pending_timers, GUINT_TO_POINTER(node_id));
}

void
controld_node_pending_timer(crm_node_t *node)
{
    ...
    if (node->when_member <= 0 || node->when_online != 0) {
        remove_node_pending_timer(node->id);
        return;
    }
    ...
    init_node_pending_timer(node->id, remaining_timeout);
}
