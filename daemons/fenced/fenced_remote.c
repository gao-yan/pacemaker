...
static int
valid_fencing_timeout(int specified_timeout, const char *scope,
                      const remote_fencing_op_t *op, const char *device)
{
    ...
    if (timeout > specified_timeout) {
        crm_warn("Specified '%s' fencing timeout %ds is too short (must be >= "
                  PCMK_OPT_STONITH_WATCHDOG_TIMEOUT " %ds) , using %ds instead",
                  scope, specified_timeout, timeout, timeout);
    }
                   ...
                   bool with_delay)
{
    ...
    timeout = valid_fencing_timeout(op->base_timeout, "global", op, device);
                     ...
                     const peer_device_info_t *chosen_peer)
{
    ...
    if (pcmk_is_set(op->call_options, st_opt_topology) && tp) {
        ...
        total_timeout = valid_fencing_timeout(op->base_timeout, "global", op,
                                              NULL);
    }
                      ...
                      peer_device_info_t *peer, const char *device)
{
    ...
    for (child = pcmk__xe_first_child(xml); child != NULL;
